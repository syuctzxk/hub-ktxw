...
API_KEY = os.getenv("OPENAI_API_KEY", "sk-247b4c94005a487b9516879348c4bcec")
...
你是一名专业的内容评审员。你的任务是对已经生成的章节内容进行质量评估，并给出具体的改进建议。

评估标准包括：
1. **准确性:** 内容是否准确反映了原始资料中的信息？
2. **完整性:** 是否涵盖了该章节应该讨论的所有重要方面？
3. **逻辑性:** 内容组织是否有条理，逻辑是否清晰？
4. **可读性:** 文字表达是否流畅，易于理解？

你需要提供一个评分（满分10分）和具体的改进建议。
"""
...
"""
    ...
    # 3. 并行处理所有章节的检索、抓取和起草
    drafted_sections = [None] * len(sections)  # 用于存储最终章节内容
    ...
    async def process_section(i, section):
        ...
        while iteration_count < max_iterations:
            iteration_count += 1
            print(f"-> 章节 '{section_title}' 第 {iteration_count} 次迭代")
            
            # 3.3.1 内容起草 (调用 Drafting Agent)
            draft_prompt = f"""
            **章节主题:** {section_title}
            ...
            **原始网页内容 (请基于此内容撰写):**
            ...
            {f"**之前收到的反馈建议:** {' '.join(feedback_history)}" if feedback_history else ""}
            ...
            try:
                ...
                section_draft = draft_result.final_output
                
                # 如果已经达到最大迭代次数，则直接使用当前版本
                if iteration_count >= max_iterations:
                    print(f"-> 章节 '{section_title}' 达到最大迭代次数，结束优化")
                    break
                    
            except Exception as e:
                error_msg = f"章节起草失败: {e}"
                print(f"-> {error_msg}")
                # 如果是第一次就失败了，就记录错误信息作为章节内容
                if iteration_count == 1:
                    section_draft = error_msg
                break
            
            # 3.3.2 评估章节内容 (调用 Evaluation Agent)
            evaluation_prompt = f"""
            请评估以下章节内容的质量：
            ...
            **章节内容:**
            ...
            请根据准确性、完整性、逻辑性和可读性四个方面进行评分（每项满分10分），
            并提供具体的改进建议。如果内容质量已经很好（总分超过35分），请回复"内容质量良好，无需进一步修改。"
            否则，请明确指出需要改进的地方。
            """
            ...
            try:
                ...
                evaluation_feedback = eval_result.final_output
                print(f"-> 章节 '{section_title}' 评估反馈:\n{evaluation_feedback}")
                
                # 检查是否需要继续迭代
                if "内容质量良好，无需进一步修改" in evaluation_feedback or "内容质量良好无需进一步修改" in evaluation_feedback:
                    print(f"-> 章节 '{section_title}' 质量达标，结束优化")
                    break
                else:
                    # 记录反馈意见，用于下一次迭代
                    feedback_history.append(evaluation_feedback)
                    
            except Exception as e:
                print(f"-> 章节 '{section_title}' 评估失败: {e}")
                # 评估失败不影响继续流程
                break
        
        # 返回章节标题和最终内容
        return i, f"## {section_title}\n\n{section_draft}"

    # 3.4 并行处理所有章节
    print("\nStep 3: 并行处理所有章节...")
    section_processing_tasks = [process_section(i, section) for i, section in enumerate(sections)]
    processed_sections = await asyncio.gather(*section_processing_tasks)
    
    # 按照原始顺序排列章节
    for i, section_content in processed_sections:
        drafted_sections[i] = section_content
