...
async def chat(user_name:str, session_id: Optional[str], task: Optional[str], content: str, tools: List[str] = []):
    ...
    if not tools or len(tools) == 0:
        agent = Agent(
            name="ChitChatAgent",
            instructions=instructions + "\n\n你是一个专门负责日常闲聊的助手。当用户的问题涉及股票或数据分析时，你应该将问题转交给StockAgent。",
            handoff_description="专门负责日常闲聊的助手。适合处理非专业话题，如问候、天气、文化、娱乐等。",
        ...
        async with mcp_server:
            ...
            agent = Agent(
                name="StockAgent",
                instructions=instructions + "\n\n你是一个专门负责股票分析和数据处理的助手。你可以访问专业的金融工具来分析股票数据。",
                handoff_description="专门负责股票分析和数据处理的助手。适合处理股票查询、财务数据分析等专业问题。",
