...
async def chat(user_name:str, session_id: Optional[str], task: Optional[str], content: str, tools: List[str] = []):
    ...
    if not tools or len(tools) == 0:
        ...
        stock_agent = Agent(
            ...
            handoff_description='负责处理所有股票问题的专家代理',
        ...
        chat_agent = Agent(
            name="chat_agent",
            instructions=get_init_message("闲聊"),
            handoff_description='负责与人闲聊专家代理',
            # mcp_servers=[mcp_server],
            model=OpenAIChatCompletionsModel(
                model=os.environ["OPENAI_MODEL"],
                openai_client=external_client,
        ...
        agent = Agent(
            name="triage_agent",
            instructions="你的名字叫任务分派助手，根据用户的提问内容，判断应该将请求分派给'chat_agent'还是'stock_agent'",
            model=OpenAIChatCompletionsModel(
                model=os.environ["OPENAI_MODEL"],
                openai_client=external_client,
            ),
    ...
    # 需要调用mcp 服务进行回答
        ...
        async with mcp_server:
            ...
            agent = Agent(
                name="triage_Agent",
                instructions="根据用户的提问内容，判断应该将请求分派给'chat_agent'还是'stock_agent'",
                model=OpenAIChatCompletionsModel(
                    model=os.environ["OPENAI_MODEL"],
                    openai_client=external_client,
                ),
