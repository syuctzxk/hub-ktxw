import asyncio
import os
os.environ["OPENAI_API_KEY"] = "sk-c1a81d43ca8a48a6924063c175460628"
...
API_KEY = os.getenv("OPENAI_API_KEY", "sk-c1a81d43ca8a48a6924063c175460628")
...
# 3.3. 反思代理 (React Agent)
react_system_prompt = """
你是一名专业的内容撰稿人。请根据主题和生成的报告内容，判断是否满足要求，json格式化输出。
如果满足，只需要输出结果：
```json
{
    "result": "ok"
...
async def research_section(section, i):
    """处理单个章节"""
...
async def final_handle(research_title, full_report_draft, questions, react_count):
    """整合报告"""
    final_prompt = f"""
    ...
    try:
        ...
        all_resport = final_report.final_output
    except Exception as e:
        return f"最终报告整合失败: {e}\n\n已完成的章节草稿:\n{full_report_draft}"
    ...
    报告内容: {all_resport}
    """
    react_response = await Runner.run(
        ReactAgent,
        react_content,
    )
    print(react_response)
    react_json = json.loads(react_response.final_output.strip("```json").strip("```"))
    if react_json["result"] == "ok":
        # 已满足要求
        return all_resport
    else:
        # 不满足要求
        ...
        if react_count >= 2:
            return all_resport
        # 反思的问题
        current_questions = react_json["questions"]
        # 重新整合
        return final_handle(research_title, full_report_draft, questions + current_questions, react_count + 1)
...
"""
    ...
    return await final_handle(full_report_draft, research_title, [], 0)
