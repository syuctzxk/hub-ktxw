...
#print(predict('1."糖醋鲤鱼怎么做啊？",2."请帮我打开百度",3."我想看昨天直播的LOL总决赛"'))

  #API路由根路径，返回欢迎信息
@app.get("/")
async def root():
  return {"message": "LLM信息抽取API服务运行中", "docs_url": "/docs"}
...
async def get_example():
    example_text = "糖醋鲤鱼怎么做啊？"
    example_result = predict(example_text)
    return {
       "input_example": example_text,
       "output_example": example_result,
       "usage": {
          "single_prediction": "POST /predict with {'text': '你的文本'}",
          "batch_prediction": "POST /batch_predict with {'texts': ['文本1', '文本2']}"
      }
     }
...
if __name__ == "main":
    import uvicorn
    uvicorn.run("作业2_提示词fastapi:app", host="0.0.0.0", port=8066, reload=True)

#Terminal 输入:uvicorn 作业2_提示词fastapi:app --reload --host 0.0.0.0 --port 8066。访问http://localhost:8066/docs
