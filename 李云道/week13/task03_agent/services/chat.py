...
async def auto_chat_with_stock(user_name: str, session_id: Optional[str], content: str, tools: List[str] = []):
    # 对话管理，通过session id
    if session_id:
        with SessionLocal() as session:
            record = session.query(ChatSessionTable).filter(ChatSessionTable.session_id == session_id).first()
    ...
    agent = Agent(
        ...
        model=OpenAIChatCompletionsModel(model=os.environ["OPENAI_MODEL"], openai_client=external_client),
        model_settings=ModelSettings(parallel_tool_calls=False)
