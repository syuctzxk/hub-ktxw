...
def process_func(example, tokenizer, max_length=384):
    ...
    # ===== Tokenize system + user 部分 =====
    instruction = tokenizer(full_prompt, add_special_tokens=False)

    # ===== Tokenize 答案部分 =====
    if len(example["answers"]["text"]) > 0 and example["answers"]["text"][0].strip():
        answer_text = example["answers"]["text"][0].strip()
    else:
        answer_text = "无法回答"
