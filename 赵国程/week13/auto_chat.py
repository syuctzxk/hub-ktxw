...
async def chat(user_name: str, task: str, session_id: Optional[str], content: str, tools: List[str] = []):
    # 对话管理，通过session id
    if session_id:
        with SessionLocal() as session:
    ...
    if not tools or len(tools) == 0:
        chat_agent, stock_agent, triage_agent = _create_agents_without_tools(
            external_client
        )

        result = Runner.run_streamed(triage_agent, input=content, session=session)  # 流式调用大模型
    ...
    # 需要调用mcp 服务进行回答
        ...
        async with mcp_server:
            chat_agent, stock_agent, triage_agent = _create_agents_with_tools(
                external_client, mcp_server, tools
            )
...
def _create_agents_without_tools(external_client: AsyncOpenAI):
    ...
    return chat_agent, stock_agent, triage_agent


def _create_agents_with_tools(external_client: AsyncOpenAI, mcp_server: MCPServerSse, tools: List[str]):
    """创建需要工具的agents"""
    # 哪些工具直接展示结果
