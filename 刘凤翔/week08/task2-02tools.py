...
class IntentDomainNerTask(BaseModel):
    ...
    entity: Dict[Literal['code', 'Src', 'startDate_dateOrig', 'film', 'endLoc_city', 'artistRole', 'location_country', 'location_area', 'author', 'startLoc_city', 'season', 'dishName', 'media', 'datetime_date', 'episode', 'teleOperator', 'questionWord', 'receiver', 'ingredient', 'name', 'startDate_time', 'startDate_date', 'location_province', 'endLoc_poi', 'artist', 'dynasty', 'area', 'location_poi', 'relIssue', 'Dest', 'content', 'keyword', 'target', 'startLoc_area', 'tvchannel', 'type', 'song', 'queryField', 'awayName', 'headNum', 'homeName', 'decade', 'payment', 'popularity', 'tag', 'startLoc_poi', 'date', 'startLoc_province', 'endLoc_province', 'location_city', 'absIssue', 'utensil', 'scoreDescr', 'endLoc_area', 'resolution', 'yesterday', 'timeDescr', 'category', 'subfocus', 'theatre', 'datetime_time'], str] = Field(description="实体")
    Src: Optional[str] = Field(description="出发地")
    Des: List[str] = Field(description="目的地")

result = ExtractionAgent(model_name = "qwen-plus").call("帮我查询一下北京到天津的汽车票", IntentDomainNerTask)
print(result)
