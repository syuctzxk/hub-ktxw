...
# 数据加载和预处理
def preprocess_data(lines, tags):
    """
        将BIO标签序列转换为单词和对应的标签。
        Args:
    ...
    for char, tag in zip(lines, tags):
        if tag.startswith('B-'):
            ...
            if current_word:
                word_tag_list.append(current_word + " : " + current_tag)
            # 重置当前词语和标签
            current_word = ''
            current_tag = ''
    ...
    if len(word_tag_list) == 0:
        word_tag_list.append('没有识别出待选的实体')
...
for lines, tags in zip(train_lines, train_tags):
    train_data.append(["".join(lines), "\n".join(preprocess_data(lines, tags))])
